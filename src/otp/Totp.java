...
public class Totp
{
    ...
    public String getOTP(long timeInMillis)
    {
        return leftPadding(hash(secret, clock.generateInterval(timeInMillis)));
    }
    ...
    public boolean verify(String otp, long time)
    {

        long code = Long.parseLong(otp);
        int intervalMilliseconds = clock.getIntervalSecond() * 1000;
        long currentInterval = time / intervalMilliseconds;


        //將來secret key改成從DB查詢
        int candidate = generate(this.secret, currentInterval);
        if (candidate == code) {
            return true;
        }
